...
//	}
	...
	$("#publishServiceBtn").click(function() {
		...
		$("#publishServiceBtn").attr("disabled","disabled");
		try {
			...
			$.ajax({
				...
				success : function(data) {
					$("#publishServiceBtn").removeAttr("disabled");
	...
	function getPageData() {
		...
		if (serviceAddr == "") {
			throw new Error("服务地址不能为空", "", 0);
			$("#serviceAddress").focus();
			return;
		}
		if ($("#serviceAddress").val() == '') {
			throw new Error("请编辑服务地址", "", 0);
			$("#serviceAddress").focus();
			return;
		}
		...
		if (!ok) {
			throw new Error("", "服务地址必须以英文字母开头且只能是英文数字下划线组合", 0);
		...
		if ($("#searchType").val() == "2" || $("#searchType").val() == "3"
			...
			sql = $("#sqlInput").val();
			if (sql == "") {
				throw new Error("sql语句不能为空", "", 0);
				return;
			}
			if ($("#tableSpace").val() == "") {
				throw new Error("数据库不能为空", "", 0);
				$("#tableSpace").focus();
				return;
			}
			chekcedSql(sql);
		...
		if (responseData.length == 0) {
			throw new Error("响应参数不能为空", "", 0);
			return;
		}
		var authType = $('input:radio[name=authKey]:checked').val();
		var logFlag = $('input:radio[name=isLog]:checked').val();
		var allowDel = $('input:radio[name=allowDel]:checked').val();
		var serviceDesc = $("#routeDesc").val();
		var table = $.getTable("requestTable");
...
function initPage(data) {
	...
	$("input[type=radio][name='allowDel'][value='1']").prop("checked", true);
	requestRow = 0;
	responseRow = 0;
	$("#requestTable tr:not(:first)").remove();
	$("#responseTable tr:not(:first)").remove();
...
function list(startPage){
	...
	var listAssembled = function(data){
		$('#pageType').val("");
...
//			}
			...
			if (content.allowDel != 0 && content.is_publish == 0 && (user == content.creator || user == "admin")) {
	...
	var listfunc = function(){
			updatePagination( ctx + '/service/getPageService',params,0,15,function(data){
	...
	var enterfunc = function(){
		$.ajax({
			...
			success : function(data) {
				...
				$("input[type=radio][name=allowDel][value=" + data.allowDel + "]").prop("checked",true);
				changeType(codetype);
				// 显示哪种参数
				showParam();
				if (searchType == 1) {
