    ...
    $.fn.autosuggest = function (options) {
        ...
        function searchQuery() {
            ...
            if (query.length >= settings.minLength) {
                ...
                if(settings.url == "/search"||undefined == settings.url || null == settings.url){
                    var jsonc = settings.data;

                    var json = suitData(query,jsonc);
                    var results = '';
                    ...
                    $(that).keydown(function (event) {
                        ...
                        if (keyCode == 13) {
                            ...
                            if (upDownOperate) {
                                ...
                                $(that).attr('realVal',getRealText($(".as-selected").data('value')));
                                $(that).val(lastText);
                                lock = false;
                                closeComponent($(that).next('.' + settings.menuClass));
        ...
        //匹配数据
        function suitData(key,data){
            var returnData = [];
            for(var i=0;i<data.length;i++){
                if(data[i].value.toLowerCase().indexOf(key.toLowerCase()) != -1 || data[i].label.toLowerCase().indexOf(key.toLowerCase()) != -1){
